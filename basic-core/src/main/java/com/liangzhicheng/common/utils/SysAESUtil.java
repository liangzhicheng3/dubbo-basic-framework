...
public class SysAESUtil {
    ...
    public static void main(String[] args) {
        String sign = aesEncrypt("key=BUqNNQ3ecKUDYofUcEn0I6qDJsMDoi3Mp4yOtTGiZaxqTYFeMUnlPNAqAJ&timeStamp="
                + System.currentTimeMillis() / 1000 + "&orderId=1122&userId=1234");
        SysToolUtil.info("加密参数:" + sign);
        String key = getParam("key", sign);
        String timeStampStr = getParam("timeStamp", sign);
        String orderId = getParam("orderId", sign);
        String userId = getParam("userId", sign);
        SysToolUtil.info("execute test -> key:" + key + ", timeStamp:" + timeStampStr
                + ", orderId:" + orderId + ", userId:" + userId);
        if(SysToolUtil.isBlank(key, timeStampStr, orderId, userId)){
            SysToolUtil.warn("参数为空");
        }else{
            if(!Constants.REQUEST_PARAM_KEY.equals(key)){
                SysToolUtil.warn("接收参数key不一致");
            }
            Long timeStamp = Long.valueOf(timeStampStr);
            if(timeStamp == 0L || System.currentTimeMillis() / 1000 - timeStamp > Constants.REQUEST_PARAM_TIME){
                SysToolUtil.warn("签名参数已过期");
            }
        }
    }
