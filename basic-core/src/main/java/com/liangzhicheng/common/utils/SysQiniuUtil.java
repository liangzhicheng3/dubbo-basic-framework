...
public class SysQiniuUtil {
    ...
    public static String generateToken(String bucket) {
        Auth auth = Auth.create(Constants.QINIU_APP_KEY, Constants.QINIU_APP_SECRET);
    ...
    public static String upload(String bucket, MultipartFile file) {
        try {
            ...
            SysToolUtil.info("sysQiniuUtil upload result : " + response.bodyString());
            ...
            try {
                ...
                SysToolUtil.error("multipartFile way QiniuException upload error : " + e.getMessage());
            }
        } catch (IOException e) {
            SysToolUtil.error("multipartFile way IOException upload error : " + e.getMessage());
            e.printStackTrace();
        }
        return null;
    }
    ...
    public static String upload(String filePath, String key) {
        ...
        try {
            ...
            SysToolUtil.error("path way sysQiniuUtil upload error : " + e.response.statusCode);
            e.printStackTrace();
        }
        return null;
    }
    ...
    public static String upload(String bucket, InputStream is) {
        try {
            ...
            Response r = ex.response;
            SysToolUtil.error(r.toString(), SysQiniuUtil.class);
            try {
                SysToolUtil.error(r.bodyString(), SysQiniuUtil.class);
            } catch (QiniuException e) {
                SysToolUtil.error("inputStream way QiniuException upload error : " + e.getMessage());
            }
        } catch (IOException e) {
            SysToolUtil.error("inputStream way IOException upload error : " + e.getMessage());
            e.printStackTrace();
        }
        return null;
    }
    ...
    public static String uploadFile(MultipartFile file){
        ...
        try{
            ...
            SysToolUtil.error("sysQiniuUtil uploadFile fail : " + e.getMessage());
            e.printStackTrace();
        }
        return fileName;
    }
