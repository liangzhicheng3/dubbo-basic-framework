...
public class SysTokenUtil {
    ...
    private static String createToken(String userId, Date expireTime){
        try{
            ...
            SysToolUtil.info("JWTCreationException : " + e.getMessage());
            e.printStackTrace();
        }
        return null;
    }
        ...
        if(SysToolUtil.isNotBlank(userId)) {
            if(SysToolUtil.isNotBlank(token)) {
                ...
                if(SysToolUtil.isNotBlank(existValue) && existValue.equals(token)) {
                    ...
                    if(!isLogin){
                        SysToolUtil.info("user isLogin userId:" + userId + ", token:" + token + ", " +
                                "existValue:" + existValue + ", verifyToken:" + isLogin);
                        SysCacheUtil.hdel(Constants.TOKEN_KEY_MAP, userId);
                    }
                    return isLogin;
                }
    ...
    public static boolean verifyToken(String userId, String token){
        ...
        try {
            ...
            SysToolUtil.warn("token 过期");
            active = false;
        } catch (JWTDecodeException exception){
            SysToolUtil.warn("token 无效");
            active = false;
        } catch (JWTVerificationException exception){
            SysToolUtil.error("token 错误");
            active = false;
        }
        return active;
    }
