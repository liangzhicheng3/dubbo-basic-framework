 ...
 * @since 2021-08-27
 */
@Service //注意是dubbo下注解,将接口暴露在外提供调用,且注册到注册中心中
@Component
...
//        XingePush.updateTokenByLogout(user.getUserId(), loginDTO.getDeviceNo(), loginDTO.getAppType());
    ...
    public TestUserVO loginMINI(TestLoginWeChatDTO loginDTO) {
        ...
        //地区处理
        TestAreaNameEntity areaNameEntity = new TestAreaNameEntity(country, province, city);
        List<Map<String, Object>> resultList = areaNameService.getArea(areaNameEntity);
        String areaName = "";
        String areaCode = "";
    ...
    private JSONObject getUserInfo(String sessionKey, String encryptedData, String iv) {
        ...
        try {
            ...
            SysToolUtil.warn("NoSuchAlgorithmException : " + e.getMessage());
            e.printStackTrace();
        } catch (NoSuchPaddingException e) {
            SysToolUtil.warn("NoSuchPaddingException : " + e.getMessage());
            e.printStackTrace();
        } catch (InvalidParameterSpecException e) {
            SysToolUtil.warn("InvalidParameterSpecException : " + e.getMessage());
            e.printStackTrace();
        } catch (IllegalBlockSizeException e) {
            SysToolUtil.warn("IllegalBlockSizeException : " + e.getMessage());
            e.printStackTrace();
        } catch (BadPaddingException e) {
            SysToolUtil.warn("BadPaddingException : " + e.getMessage());
            e.printStackTrace();
        } catch (UnsupportedEncodingException e) {
            SysToolUtil.warn("UnsupportedEncodingException : " + e.getMessage());
            e.printStackTrace();
        } catch (InvalidKeyException e) {
            SysToolUtil.warn("InvalidKeyException : " + e.getMessage());
            e.printStackTrace();
        } catch (InvalidAlgorithmParameterException e) {
            SysToolUtil.warn("InvalidAlgorithmParameterException : " + e.getMessage());
            e.printStackTrace();
        } catch (NoSuchProviderException e) {
            SysToolUtil.warn("NoSuchProviderException : " + e.getMessage());
            e.printStackTrace();
        }
        return null;
    }
     ...
     */
    @Override
    public TestUserVO saveUser(TestUserDTO userDTO) {
        String username = userDTO.getUsername();
        String phone = userDTO.getPhone();
        if(SysToolUtil.isBlank(username, phone)){
            throw new TransactionException(ApiConstant.PARAM_IS_NULL);
        }
        TestUserEntity user = new TestUserEntity(SysToolUtil.random(), username,
                "123456", phone, "101", "中国",
                "10119", "广东", "1011917", "东莞", "999");
        baseMapper.insert(user);
        return SysBeanUtil.copyEntity(user, TestUserVO.class);
    }
     ...
     */
    @Override
    public TestUserVO getUser(TestUserDTO userDTO) {
        String userId = userDTO.getId();
        SysToolUtil.info("接收到{" + userId + "}号用户的请求,接下来调用user-provider用户供应者服务查询此用户信息");
        TestUserEntity user = baseMapper.selectById(userId);
        Optional.ofNullable(user).orElseThrow(() ->
                new TransactionException(ApiConstant.USER_NOT_EXIST));
